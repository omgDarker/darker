...
public class ArticleController {
    ...
    public ModelAndView findArticleByClassifyId(@RequestParam(value = "pageNum", required = false, defaultValue = "1") Integer pageNum,
        ...
        // 文章总条数
        int count = SpringBootService.getArticleService().selectCount(new EntityWrapper<ArticleDO>().where("classify_id={0}", classifyId));
        // 根据条件查询文章
        List<ArticleDO> list = SpringBootService.getArticleService().selectPage(new Page<>(pageNum, pageSize), new EntityWrapper<ArticleDO>().where("classify_id={0}", classifyId)).getRecords();
        // 处理文章简介长度
        list.forEach(opt -> {
    ...
    public ModelAndView findArticleByClassifyIdAndColumnId(@RequestParam(value = "pageNum", required = false, defaultValue = "1") Integer pageNum,
        ...
        ModelAndView modelAndView = new ModelAndView("home/article");
        // 文章总条数
        int count = SpringBootService.getArticleService().selectCount(new EntityWrapper<ArticleDO>().where("classify_id={0} ", classifyId).and("column_id={0}", columnId));
        // 文章列表
        modelAndView.addObject("list", SpringBootService.getArticleService().selectPage(new Page<>(pageNum, pageSize), new EntityWrapper<ArticleDO>().where("classify_id={0} ", classifyId).and("column_id={0}", columnId)).getRecords());
        // 分类ID
        modelAndView.addObject("classifyId", classifyId);
        // 分类名称
        modelAndView.addObject("classifyName", ConvertAttribute.getClassifyMap().getOrDefault(classifyId, "其他分类"));
